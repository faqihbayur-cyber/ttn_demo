...
window.initHomeView = function(){
  ...
  const wrapper = document.querySelector(".home-customer-wrapper");
    ...
    try {
      ...
      try {
        ...
        if (kantorData && customerLat && customerLng) {
          ...
          console.log("📍 lokasiCabang normalized:", loc);
          if (loc) {
      ...
      if (window.fotoBase64Home) {
        foto = await compressImageHome(window.fotoBase64Home);
      }
      ...
      // UPDATE idCustomer ke doc yang sama
      await window.updateDoc(docRef, { idCustomer });
